...
class ProjectTask(models.Model):
    ...
    @api.depends('timesheet_ids.unit_amount', 'timesheet_ids.unit_amount_invoiced', 'closed_budget', 'non_invoiceable', 'child_ids.total_hours_invoiced', 'child_ids.work_efficiency')
    def _compute_efficiency(self):
        for task in self.sorted(key=lambda t: str(t.id), reverse=True):
            ...
            task.work_efficiency = ((eff_invoiced / eff_worked) * 100 if eff_worked > 0 else 0.0)